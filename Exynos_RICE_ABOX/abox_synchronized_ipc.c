...
int ff_prot_spk_read(void *prm_data, int offset, int size)
{
	...
	if(!data)
	{
		pr_err("[Iron Device-APS:%s] data is NULL", __func__);
		return ret;
	}
...
int ff_prot_spk_write(void *prm_data, int offset, int size)
{
	...
	if(!data)
	{
		pr_err("[ID-APS:%s] data is NULL", __func__);
		return ret;
	}
	...
	case REALTIME_EXTRA:
			...
			if(erap_msg->param.raw.params[0] == VENDOR_IRONDEVICE_ID) {
				if(erap_msg->param.raw.params[1] == RD_DATA) {
					...
					ipc_dbg("read_avail after parital read[%d]",
							abox_ipc_read_avail);
					...
					ipc_dbg("write_avail after parital read[%d]",
							abox_ipc_write_avail);
