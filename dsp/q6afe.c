...
#endif /*CONFIG_TAS25XX_ALGO*/
#if IS_ENABLED(CONFIG_ID_APS_ALGO)
static int32_t sma_aps_algo_callback(uint32_t opcode, uint32_t *payload,
					uint32_t payload_size);
#endif /*CONFIG_ID_APS_ALGO*/
...
#endif /*CONFIG_TAS25XX_ALGO*/
#if IS_ENABLED(CONFIG_ID_APS_ALGO)
	struct afe_id_aps_calib_get_resp sma_calib_data;
	uint32_t sma_module_rx;
#endif /*CONFIG_ID_APS_ALGO*/
#ifdef CONFIG_SEC_SND_ADAPTATION
...
#if IS_ENABLED(CONFIG_ID_APS_ALGO)
void afe_sma_aps_init(uint32_t module_id_rx)
{
	this_afe.sma_module_rx = module_id_rx;
}
EXPORT_SYMBOL(afe_sma_aps_init);
					...
					uint32_t payload_size)
{
	...
	case AFE_PORT_CMDRSP_GET_PARAM_V3:
		...
		break;
	default:
		pr_err("[ID-APS:%s] Unrecognized command %d\n", __func__, opcode);
		return -EINVAL;
	}
	data_dest = (u32 *) &this_afe.sma_calib_data;
	data_dest[0] = payload[0];
	memcpy(&data_dest[1], &param_hdr, sizeof(struct param_hdr_v3));
	memcpy(&data_dest[5], data_start, param_hdr.param_size);
	...
	if (!data_dest[0]) {
		...
		pr_debug("[ID-APS:%s] status: %d", __func__, data_dest[0]);
		atomic_set(&this_afe.state, -1);
	}
	return 0;
}
		...
		int32_t length, uint8_t *data, uint16_t port_id)
{
	...
	if (!data || length <= 0) {
		pr_err("[ID-APS:%s] Invalid params, length: %d\n",
			__func__, length);
		goto fail_cmd;
	}

	pr_info("[ID-APS:%s] module id : 0x%x ", __func__, module_id);
	if (module_id == this_afe.sma_module_rx) {
		port = port_id;
	} else {
		pr_err("[ID-APS:%s] invalid module id %d\n", module_id);
		goto fail_cmd;
	}
	memset(&param_hdr, 0, sizeof(param_hdr));
	param_hdr.module_id = module_id;
	param_hdr.instance_id = INSTANCE_ID_0;
	param_hdr.param_id = param_id;
	param_hdr.param_size = sizeof(struct afe_id_aps_get_calib);
	ret = q6afe_get_params(port, NULL, &param_hdr);
	if (ret < 0) {
		pr_err("[ID-APS:%s] get param port 0x%x param id[0x%x]failed %d\n",
			__func__, port, param_hdr.param_id, ret);
		goto fail_cmd;
	}

	memcpy(data, &this_afe.sma_calib_data.res_cfg, length);

	ret = 0;
fail_cmd:
	return ret;
}
EXPORT_SYMBOL(afe_sma_aps_get_calib_data);
				...
				int32_t length, uint8_t *data, uint16_t port_id)
{
	...
	pr_info("[ID-APS:%s] length: %d\n", __func__, length);
	if (!data || (length < 0)) {
		pr_err("[ID-APS:%s] Invalid params\n", __func__);
		return ret;
	}

	pr_info("[ID-APS:%s] module id : 0x%x\n", __func__, module_id);
	if (module_id == this_afe.sma_module_rx) {
		port = port_id;
	} else {
		pr_err("[ID-APS:%s] invalid module id %d\n", module_id);
	...
	ret = q6common_pack_pp_params(packed_param_data, &param_hdr,
			(u8 *) data, &single_param_size);
	if (ret) {
		pr_err("[ID-APS:%s] Failed to pack param data, error %d\n", __func__,
		       ret);
		goto done;
	}
	packed_param_size += single_param_size;
...
EXPORT_SYMBOL(afe_sma_aps_set_calib_data);
#endif
